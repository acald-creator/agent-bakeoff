...
import { loadAllNotes, persistNote, deleteNote as storageDeleteNote } from './persistence';
...
const [state, setState] = createStore<NotesState>({
  notes: loadAllNotes(),           // Hydrate from localStorage index on startup
  activeNoteId: parseHashNoteId(), // Restore from URL hash on reload
  searchQuery: '',
});
...
export function setActiveNoteId(id: string | null): void {
  ...
  // Use replaceState to avoid polluting browser history with every note click
  if (id) {
