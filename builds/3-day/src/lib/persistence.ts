 ...
 *
 * Design decisions:
 *   1. SPLIT STORAGE: Index (metadata) and body (full content) are separate
 *      localStorage keys. Sidebar reads metadata (~150 bytes/note) on startup.
 *      Bodies are loaded on demand — a 10 KB note is only read when opened.
 *
 *   2. KEY SCHEME:
 *      - Index:  "ink:index"         → Record<id, NoteIndexEntry>
 *      - Bodies: "ink:note:<id>"     → NotePayload { title, body, updatedAt }
 *
 *   3. SILENT FAILURE: localStorage can throw in private browsing or when full.
 *      All writes are wrapped in try/catch. App continues in-memory if needed.
 *
 *   4. COLLAB READINESS: updatedAt on NotePayload for LWW merge in future.
 */
...
/**
 * Export all notes as a JSON blob for backup.
